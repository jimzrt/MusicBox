...
{
public:
    DFMiniMp3(T_SERIAL_METHOD& serial, T_NOTIFICATION_METHOD& notifier) :
        _serial(serial),
        _notifier(notifier),
...
private:
    ...
    T_NOTIFICATION_METHOD& _notifier;
        ...
        do
        {
            if (readPacket(&replyCommand, &replyArg))
            {
                ...
                else
                {
                    ...
                    case 0x3d: // micro sd
                    case 0x3c: // usb
                        _notifier.OnPlayFinished(replyArg);
                        break;
                    ...
                    case 0x3F:
                        ...
                        {
                            _isOnline = true;
                       //     _notifier.OnCardOnline(replyArg);
                        }
                        else if (replyArg & 0x01)
                        {
                            _isOnline = true;
                       //     _notifier.OnUsbOnline(replyArg);
                        }
                        break;
                    ...
                    case 0x3A:
                        if (replyArg & 0x02)
                        {
                     //       _notifier.OnCardInserted(replyArg);
                        }
                        else if (replyArg & 0x01)
                        {
                   //         _notifier.OnUsbInserted(replyArg);
                        }
                        break;
                    ...
                    case 0x3B:
                        if (replyArg & 0x02)
                        {
                  //          _notifier.OnCardRemoved(replyArg);
                        }
                        else if (replyArg & 0x01)
                        {
                    //        _notifier.OnUsbRemoved(replyArg);
                        }
                        break;

                    case 0x40:
                        _notifier.OnError(replyArg);
                        return 0;
                        break;
